...
export function createPatchFunction(backend) {
  ...
  function createElm(
    ...
    // 有vnode.elm，表示之前渲染过，ownerArray表示有子节点
  ...
  return function patch(oldVnode, vnode, hydrating, removeOnly) {
    ...
    let isInitialPatch = false // 记录新创建的dom元素没有挂载到页面，而是存储在内存中
    const insertedVnodeQueue = []

    // 老的 VNode 不存在，场景：调用组件的$mount时，只是将创建的dom存储在内存中，并没有挂载在页面上
    if (isUndef(oldVnode)) {
      // empty mount (likely as component), create new root element
      // 记录新创建的dom元素没有挂载到页面，而是存储在内存中
      isInitialPatch = true
      // 创建新的 VNode
      ...
      // 判断参数1-oldVnode 是否是真实 DOM，不是真实 DOM
